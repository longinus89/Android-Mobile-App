...
public class ARShotActivity extends Activity implements SensorEventListener, OnTouchListener{
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		try {
			...
			if(settings.getBoolean("firstAccess",false)==false){
				...
				//dataSourceEditor.putString("DataSource1", "Wikipedia|http://ws.geonames.org/findNearbyWikipediaJSON|0|0|true");
				//dataSourceEditor.putString("DataSource2", "Twitter|http://search.twitter.com/search.json|2|0|true");
				...
				dataSourceEditor.putString("DataSource0", "Own URL|http://mixare.org/geotest.php|4|0|false");
	...
	public boolean onOptionsItemSelected(MenuItem item){
		...
		case 1:		
			if(!dataView.isLauncherStarted()){
				MixListView.setList(4);
				Intent intent = new Intent(ARShotActivity.this, DataSourceList.class); 
...
class AugmentedView extends View {
	...
	protected void onDraw(Canvas canvas) {
		try {
			...
			ARShotActivity.dWindow.setWidth(canvas.getWidth());
			ARShotActivity.dWindow.setHeight(canvas.getHeight());

			ARShotActivity.dWindow.setCanvas(canvas);

			if (!ARShotActivity.dataView.isInited()) {
				ARShotActivity.dataView.init(ARShotActivity.dWindow.getWidth(), ARShotActivity.dWindow.getHeight());
			}
			...
			ARShotActivity.dataView.draw(ARShotActivity.dWindow);
		} catch (Exception ex) {
			app.doError(ex);
		}
	}
