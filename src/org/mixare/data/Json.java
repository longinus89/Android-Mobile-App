...
import it.pdm.nodeshotmobile.ARShotActivity;
...
public class Json extends DataHandler {
	...
	public List<Marker> load(JSONObject root, DataSource datasource) {
		...
		try {
			...
			if (dataArray != null) {
				...
				Log.i(ARShotActivity.TAG, "processing " + datasource.getType()
						+ " JSON Data Array");
	...
	public Marker processTwitterJSONObject(JSONObject jo, DataSource datasource)
		...
		if (jo.has("geo")) {
			...
			if (lat != null) {
				Log.v(ARShotActivity.TAG, "processing Twitter JSON object");
	...
	public Marker processMixareJSONObject(JSONObject jo, DataSource datasource) throws JSONException {
		...
		if (jo.has("title") && jo.has("lat") && jo.has("lng")
			...
			Log.v(ARShotActivity.TAG, "processing Mixare JSON object - " +
					"Name="+jo.getString("title")+" lat="+jo.getString("lat")+" lng="+jo.getString("lng"));
			String link=null;
	...
	public Marker processWikipediaJSONObject(JSONObject jo, DataSource datasource)
		...
		if (jo.has("title") && jo.has("lat") && jo.has("lng")
			...
			Log.v(ARShotActivity.TAG, "processing Wikipedia JSON object");
