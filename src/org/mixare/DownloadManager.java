...
public class DownloadManager implements Runnable {
		...
		try {
			if(ctx!=null && request!=null && ctx.getHttpGETInputStream(request.source.getUrl())!=null){
				...
				try {
					...
					Log.v(ARShotActivity.TAG, "try to load JSON data");

					JSONObject root = new JSONObject(tmp);

					Log.d(ARShotActivity.TAG, "loading JSON data");
					...
					Log.v(ARShotActivity.TAG, "no JSON data");
					Log.v(ARShotActivity.TAG, "try to load XML data");
					...
					try {
						...
						Log.i(ARShotActivity.TAG, "loading XML data");	
	...
	public synchronized String submitJob(DownloadRequest job) {
		if(job!=null) {
			...
			Log.i(ARShotActivity.TAG, "Submitted Job with " + jobId + ", type: "
